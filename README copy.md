...
- Admin-only routes guarded by `admin_middleware`
...
- **Read receipts** — single check (sent) / double check (read) indicators
- **Date separators** — messages grouped by day (Today, Yesterday, full date)
- Duplicate conversation prevention — `createOrGetConversation` pattern
...
- Soft delete for users — preserves chat history and referential integrity

### Permission System
- CRUD operations for permission definitions
- User-permission assignment model (per-user granular permissions)
- Duplicate permission name detection (case-insensitive)
- Admin-only access for permission listing
...
| **Permission** | `permission_name`, `is_default` | — | Permission definitions |
| **UserPermission** | `userId`, `permissions[{permission_name, permission_value[]}]` | — | Per-user permission assignments |
...
| GET | `/permission/getPermission` | Yes | admin | List all permissions (admin only) |
...
| [walkthrough.md](walkthrough.md) | End-to-end code flow walkthrough — auth, socket, messaging, and frontend flows |
| [fixes_documentation.md](fixes_documentation.md) | Changelog of 15 bug fixes across 14 files |
